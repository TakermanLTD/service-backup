...
namespace Takerman.Backups.Services
{
    public class SyncService(ILogger<SyncService> _logger) : ISyncService
    {
        public string Sync()
        {
            try
            {
                var result = "sshpass -p 'Hakerman91!' ssh -p 1991 root@takerman.net && rclone sync /home/takerman/volumes/mssql/data/ google-drive:projects/backups && dupdate".ExecuteCommand();
