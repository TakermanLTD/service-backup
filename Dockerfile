...
EXPOSE 80
EXPOSE 443
RUN apt update && apt install -y curl gnupg libpng-dev libjpeg-dev curl libxi6 build-essential libgl1-mesa-glx openssh-server sshpass
...
ENV ASPNETCORE_ENVIRONMENT=Production
WORKDIR /src
RUN apt update && apt install -y curl libpng-dev libjpeg-dev curl libxi6 build-essential libgl1-mesa-glx openssh-server sshpass
RUN curl -fsSL https://deb.nodesource.com/nsolid_setup_deb.sh | sh -s 20
RUN apt-get install -y nodejs
